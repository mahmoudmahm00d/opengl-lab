...
int LoadGLTextures() // Load Bitmaps And Convert To Textures
{
	...
	if ((TextureImage[0] = LoadBMP("Soledad89OpenGLNehe demos/CrazyGridData/Font.bmp")) && // Load The Font
		(TextureImage[1] = LoadBMP("Soledad89OpenGLNehe demos/CrazyGridData/tiger.bmp"))) // Load Background Image (change to tiger)
	{
		...
		for (loop1 = 0; loop1 < 2; loop1++) // Loop Through 2 Textures
		{
			...
			glTexImage2D(GL_TEXTURE_2D, 0, 3, TextureImage[loop1]->sizeX, TextureImage[loop1]->sizeY, 0, GL_RGB,
			             GL_UNSIGNED_BYTE, TextureImage[loop1]->data);
                   ...
                   int nCmdShow) // Window Show State
{
	...
	while (!done) // Loop That Runs While done=FALSE
	{
		...
		else // If There Are No Messages
		{
			...
			if (!gameover && active) // If Game Isn't Over And Programs Active Move Objects
			{
				for (loop1 = 0; loop1 < (stage * level); loop1++) // Loop Through The Different Stages
				{
					...
					if ((enemy[loop1].fx == player.fx) && (enemy[loop1].fy == player.fy))
					{
						...
						PlaySound("CrazyGridData/Die.wav", NULL, SND_SYNC); // Play The Death Sound
					}
				}
			...
			{
				PlaySound("CrazyGridData/Complete.wav", NULL, SND_SYNC); // If So, Play The Level Complete Sound
			...
			if ((player.fx == hourglass.x * 60) && (player.fy == hourglass.y * 40) && (hourglass.fx == 1))
			{
				...
				PlaySound("CrazyGridData/freeze.wav", NULL, SND_ASYNC | SND_LOOP);
			...
			// Is The hourglass fx Variable Equal To 0 And The fy
			{
				...
				PlaySound("CrazyGridData/hourglass.wav", NULL, SND_ASYNC); // If So, Play The Hourglass Appears Sound
